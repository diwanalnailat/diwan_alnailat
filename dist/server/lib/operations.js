...
export async function readiness(env, c) {
  ...
  const components = Object.entries(INTEGRATION_REQUIREMENTS).map(
    ([id, keys]) => ({
      id,
      status: flags[id] ? "configured_unverified" : "needs_configuration",
  ...
  return {
    ...
    external_gates: [
      "تجربة إرسال رمز ورسالة واتساب فعليين بعد تهيئة الحساب والقوالب",
...
export async function maintain(env) {
  ...
  const lease = await db
    .prepare(
      "INSERT INTO nl_integration_limits(key,hits,expires) VALUES('maintenance:lease',1,?) ON CONFLICT(key) DO UPDATE SET expires=excluded.expires WHERE expires<? RETURNING key",
    )
    .bind(at + 60000, at)
    .first();
  ...
  await db
    .prepare(
      "INSERT INTO nl_settings(key,value,version) VALUES('maintenance:last',?,1) ON CONFLICT(key) DO UPDATE SET value=excluded.value,version=version+1",
    )
